...
class CustomPullerEnv(MujocoEnv, utils.EzPickle):
    ...
    def reset_model(self, push_direction = None):
        ...
        R = 0.8  # The side length of the table is 1
        target_r = np.random.uniform(0.6 * R, 0.8 * R)
        cylinder_r = np.random.uniform(target_r + (0.15 * R), R)
